...
public class UserManager : UserManager<RsaUser>, IUserManager
{
    ...
    public Task<IdentityResult> AddToRoleAsync(IUser user, string role) => base.AddToRolesAsync(user as RsaUser, new List<string> { role });
    public Task<bool> CheckPasswordAsync(IUser user, string password) => base.CheckPasswordAsync(user as RsaUser, password);
    public Task<IdentityResult> CreateAsync(IUser user, string? password = null) => base.CreateAsync(user as RsaUser, password);
    public Task<IList<string>> GetRolesAsync(IUser user) => base.GetRolesAsync(user as RsaUser);
